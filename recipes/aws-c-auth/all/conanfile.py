...
class AwsCAuth(ConanFile):
    ...
    def requirements(self):
        ...
        elif Version(self.version) < "0.7.2":
            self.requires("aws-c-io/0.13.4", transitive_headers=True)
            self.requires("aws-c-http/0.6.22", transitive_headers=True)
        else:
            self.requires("aws-c-io/0.13.30", transitive_headers=True)
            self.requires("aws-c-http/0.7.11", transitive_headers=True)
        if Version(self.version) >= "0.7.2":
            self.requires("aws-c-sdkutils/0.1.12", transitive_headers=True)
        elif Version(self.version) >= "0.6.5":
